...
func on_use(_usedby):
	if mode == RigidBody3D.FREEZE_MODE_STATIC:
		# Static item in the world
		if filename == "res://Items/Torch.tscn":
			activated = !activated
			if activated:
				torch_ignite()
			else:
		...
		elif filename == "res://Items/Lantern.tscn":
			activated = !activated
			if activated:
				lantern_ignite()
			else:
...
func on_drop():
	print("PhysicalItem.gd: ", name,"-",filename, " dropped.")
	if filename == "res://Items/Torch.tscn":
		torch_extinguish()
	if name == "res://Items/Lantern.tscn":
		lantern_extinguish()
	pass
